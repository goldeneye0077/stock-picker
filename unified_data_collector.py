...
class UnifiedDataCollector:
    ...
    async def collect_daily_data_unified(self, trade_date: str, filtered_stocks: List[Tuple[str, str]]) -> int:
        ...
        try:
            # 使用批量接口获取数据
            def _get_daily_data():
                df = asyncio.run(self.tushare_client.get_daily_data_by_date(trade_date))
                return df

            df = sync_collect_with_retry(_get_daily_data)
    ...
    async def collect_moneyflow_data_unified(self, trade_date: str, filtered_stocks: List[Tuple[str, str]]) -> int:
        ...
        try:
            # 使用批量接口获取数据
            def _get_moneyflow_data():
                df = asyncio.run(self.tushare_client.get_moneyflow_by_date(trade_date))
                return df

            df = sync_collect_with_retry(_get_moneyflow_data)
    ...
    async def get_trading_days(self, days: int = 7) -> List[str]:
        ...
        try:
            def _get_trading_days():
                ...
                # 获取交易日历
                cal_df = asyncio.run(self.tushare_client.get_trade_cal(
                    start_date.strftime('%Y%m%d'),
                    end_date.strftime('%Y%m%d')
                ))
                return cal_df

            cal_df = sync_collect_with_retry(_get_trading_days)
